...
async function updateProjectBudget (req, res) {
  try {
    const projectId = Number(req.params.id)
    const data = req.body
    const { isValid, message } = validateDataWithStructure({ projectId, ...data }, PROJECT_DATA_STRUCTURE)
...
async function getBudgetDetails (req, res) {
  try {
    ...
    const response = await executeDbQuery(buildSelectQuery('project', '*', { year: data.year, projectName: data.projectName }))
    if (!response.success || !response?.result?.length || !response?.result) {
      return res.status(400).json({
        success: false,
